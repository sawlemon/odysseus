...
const PDF_FILTER_KEY = 'odysseus-model-pdf-filter';
...
// ── PDF-capable filter (models with native PDF file input) ──
function _getPdfFilter() {
  return Storage.get(PDF_FILTER_KEY, '') === '1';
}
function _setPdfFilter(on) {
  Storage.set(PDF_FILTER_KEY, on ? '1' : '');
}
...
function _buildModelRow(mid, url, displayName, endpointId, offline, modelType, pdfNative) {
  ...
  if (pdfNative) {
    const badge = document.createElement('span');
    badge.className = 'model-type-badge model-pdf-badge';
    badge.textContent = 'PDF';
    badge.title = 'Accepts PDFs natively (reads scanned/image pages directly)';
    badge.style.cssText = 'font-size:0.65em;padding:1px 4px;border-radius:3px;background:color-mix(in srgb, var(--accent, #b91c1c) 75%, black);color:#fff;margin-left:6px;vertical-align:middle;';
    span.appendChild(badge);
  }
...
export async function refreshModels(force = false) {
  ...
  try {
    ...
    const pdfFilterOn = _getPdfFilter();
    // Whether the server marked any model as native-PDF at all — the chip is
    // pointless (and misleading) before the capability catalog has loaded.
    let anyPdfCapable = false;
    ...
    if (_cachedItems && _cachedItems.length > 0) {
      _cachedItems.forEach(item => {
        ...
        const pdfFlags = item.models_pdf || [];
        const extraPdfFlags = item.models_extra_pdf || [];
        const epModelType = item.model_type || 'llm';
        (item.models || []).forEach((mid, i) => {
          const pdfNative = !!pdfFlags[i];
          if (pdfNative) anyPdfCapable = true;
          if (pdfFilterOn && !pdfNative) return;
        ...
        (item.models_extra || []).forEach((mid, i) => {
          const pdfNative = !!extraPdfFlags[i];
          if (pdfNative) anyPdfCapable = true;
          if (pdfFilterOn && !pdfNative) return;
    ...
    if (favs.length > 0) {
      ...
      if (favModels.length > 0) {
        ...
        if (!favCollapsed) {
          ...
          favModels.forEach(({ mid, url, displayName, endpointId, offline, modelType, pdfNative }) => {
            favContainer.appendChild(_buildModelRow(mid, url, displayName, endpointId, offline, modelType, pdfNative));
          });
    ...
    categoryOrder.forEach(({ key, label }) => {
      ...
      Object.entries(endpoints).forEach(([epName, epModels]) => {
        ...
        visible.forEach(({ mid, url, displayName, endpointId, offline, modelType, pdfNative }) => {
          target.appendChild(_buildModelRow(mid, url, displayName, endpointId, offline, modelType, pdfNative));
        });
        ...
        if (allHidden.length > 0) {
          ...
          showMoreBtn.addEventListener('click', () => {
            showMoreBtn.remove();
            allHidden.forEach(({ mid, url, displayName, endpointId, offline, modelType, pdfNative }) => {
              target.appendChild(_buildModelRow(mid, url, displayName, endpointId, offline, modelType, pdfNative));
            });
          });
    ...
    if (totalModelCount >= 10) {
      ...
      searchBox.addEventListener('input', () => {
        ...
        (_cachedItems || []).forEach(item => {
          ...
          const allPdf = (item.models_pdf || []).concat(item.models_extra_pdf || []);
          allModels.forEach((mid, i) => {
            const display = allDisplay[i] || mid;
            const pdfNative = !!allPdf[i];
            if (pdfFilterOn && !pdfNative) return;
            if (!mid.toLowerCase().includes(q) && !display.toLowerCase().includes(q)) return;
            searchResults.appendChild(
              _buildModelRow(mid, item.url, display, item.endpoint_id || null, false, item.model_type || 'llm', pdfNative)
            );
          });
        });
    ...
    // ── PDF-capable filter chip (models with native PDF file input) ──
    if ((anyPdfCapable || pdfFilterOn) && _cachedItems && _cachedItems.length > 0) {
      const chip = document.createElement('div');
      chip.className = 'model-pdf-filter-chip';
      chip.textContent = 'PDF-capable only';
      chip.title = pdfFilterOn
        ? 'Showing only models that accept PDFs natively — click to show all models'
        : 'Show only models that accept PDFs natively (read scanned/image pages directly)';
      chip.style.cssText = 'display:inline-block;margin:2px 0 6px;padding:2px 8px;font-size:0.78em;'
        + 'border:1px solid var(--border,#444);border-radius:10px;cursor:pointer;user-select:none;'
        + (pdfFilterOn
          ? 'background:var(--accent,#b91c1c);color:#fff;border-color:transparent;'
          : 'opacity:0.65;');
      chip.addEventListener('click', (e) => {
        e.stopPropagation();
        _setPdfFilter(!pdfFilterOn);
        refreshModels();
      });
      box.insertBefore(chip, box.firstChild);

      if (pdfFilterOn && localCount + apiCount === 0) {
        const none = document.createElement('div');
        none.style.cssText = 'text-align:center;padding:12px;opacity:0.5;font-size:0.85em;';
        none.textContent = 'No native-PDF models on your endpoints — click the chip to show all.';
